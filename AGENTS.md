...
> holds only a `.gitkeep`, and the files the process expects in them are on `example`. Replace this
> banner with your own status when you take the repository over; it is the same line as your
> `README.md`'s.
